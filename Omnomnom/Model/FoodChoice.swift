...
nonisolated struct FoodChoice: Identifiable, Hashable, Sendable {
    /// Where the item lives: the bundled database, a custom `Food` row, or a `Recipe`.
    ...
    init(source: Source, name: String, perUnit: Nutrition, gramsPerServing: Double? = nil, lastAmount: Double? = nil) {
        self.source = source
        self.name = name
    ...
    func with(lastAmount: Double?) -> FoodChoice {
        FoodChoice(source: source, name: name, perUnit: perUnit, gramsPerServing: gramsPerServing, lastAmount: lastAmount)
    }
