...
struct RecipeSnapshotTests {
    ...
    private func snapshot(
        of ingredients: [(amount: Double, measure: FoodMeasure, per100: Nutrition)],
        servings: Double, logged: Double
    ) -> Nutrition {
    ...
    /// Oats by weight and milk by volume: the nutrition is the same arithmetic either way.
    private var rows: [(amount: Double, measure: FoodMeasure, per100: Nutrition)] {
        [(amount: 40, measure: .mass, per100: oats), (amount: 200, measure: .volume, per100: milk)]
    }
    ...
    @Test func editingIngredientsAfterwardsLeavesTheSnapshotAlone() {
        ...
        ingredients[0].amount = 80
        ingredients.append((amount: 30, measure: .mass, per100: Nutrition(energy: 400, sugar: 100)))
    ...
    @Test func changingServingsAfterwardsLeavesTheSnapshotAlone() {
        let frozen = snapshot(of: rows, servings: 2, logged: 1)
        let after = snapshot(of: rows, servings: 4, logged: 1)
    ...
    @Test func snapshotIsPerServingTimesServingsLogged() {
        let perServing = RecipeMath.perServing(total: RecipeMath.total(ingredients: rows).nutrition, servings: 2)
